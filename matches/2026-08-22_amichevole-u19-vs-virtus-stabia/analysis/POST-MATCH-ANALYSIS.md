# Post-match analysis — Napoli Primavera vs Virtus Stabia

**Match:** Amichevole · 22 August 2026  
**Score:** 2–0 (Favicchio 18' — assist Baridó; Mané 70' — assist Kenmogne)  
**Shots:** 14 (8 on target) · **xG:** ~1.90  
**PPDA:** Napoli **8.00** (ultra-high) · Virtus Stabia **19.38** (low block)  
**Analyst:** Francesco Polverino (staff consensus)

---

## Overall verdict

Best friendly of the pre-season so far. Progressive, vertical passing and combination play created a high volume of chances; the high defence, though only briefly trained, recovered the ball high (PPDA 8.0) and fed counters. The gap is still finishing, final-third decision-making, and disciplined cover when the wing-back presses.

---

## What went well

1. **Passing — vertical, progressive, successful**  
   More line-breaking, higher completion in progressive actions, and progressions that led to goals. Combination quality improved.

2. **Flank combinations**  
   - **1st half — right:** Baridó–Melnyk  
   - **2nd half — left:** Kenmogne with Liccardo (after Liccardo swapped roles with Chiaiese)

3. **High defence / PPDA**  
   Aggressive despite limited training. Global PPDA **8.00** (1H 7.63 · 2H 8.33) vs Stabia **19.38** — ultra-high press vs low block. Opponent passes allowed in our defensive 60%: 136 vs 310. Defensive actions in their half: 17. Produced counter-attacks.

4. **Chance creation**  
   Clear technical improvement — many chances created.

5. **Dictate vs counter**  
   Good understanding of when to manage possession and when to go. Quicker reactions to win the ball back.

---

## Even better if

1. **Counter-press longevity (6–7 seconds)**  
   Press reaction is good; the press itself often lasts ~4–5s. Once they reorganise, we stop. Opponent went quite direct, which made light presses less effective. Next week’s quickness work (short sprints, not long efforts) should support this.

2. **Convert chances**  
   14 shots, 8 on target, xG ~1.90 — luckily matched with two goals, but four 1v1s (including a 1v0 / open goal) must be finished. Number 10s, especially the creative ones, need to be more instinctive and shoot more from range.

3. **No. 10 positioning**  
   Some 10s drift central instead of staying wide (hiding from play, or over-centralising). Stay in position; only move when the ball is on your side. Opposite-side central occupancy only works if a midfielder fills and pushes — and we want to keep the midfield box, so stick to the role.

4. **2nd-half midfield tempo**  
   Ball circulation had good moments, but midfielders need more progressive risk: diagonal, or long over the top when the striker is 1v1 with a centre-back. Don’t always wait for the perfect pass — if it fails, win it back.

5. **Zappalà–Branchizio: delay and hold**  
   Both need to **delay and hold**, not over-press. Zappalà is often over-eager to jump; when he presses, Branchizio must **delay and hold cover** (not shuffle into man-to-man) — otherwise space opens and balls go over the top. In **1v1**, Zappalà should delay rather than dive in for the ball; going straight for the press is how he gets beaten easily. Same after turnovers: chasing without connection leaves the back line exposed.

---

## Staff notes (Monteleone / De Santis) — aligned themes

- Low/mid block already in good shape; next focus is **last 20–30m** and attacking the goal.  
- Fill the box on wide balls / crosses; striker attacks the goal, not the wide player; opposite TQ / quinto inside the goal mouth.  
- Zappalà now reaches the area — train how to attack, insert, and enter the box.  
- Sagome + defenders → rondo → finalize.
